...
import { AVATAR_COLORS, DEFAULT_PROFILE_NAME, MAX_PROFILES, type AvatarColor, type Profile } from '../shared/profiles'
...
/**
 * Manages the set of saved profiles (up to `MAX_PROFILES`), each with its own
 * settings/secrets directory under userData/profiles/<id>/ (see ConfigStore).
 * Switching, or deleting the active profile, requires every subsystem built
 * from the old profile's config to be rebuilt from the new one — see
 * `reinitializeForActiveProfile` in index.ts, which the caller runs after
 * `setActive`/`delete` here.
 */
export class ProfileManager {
  ...
  private load(userDataDir: string): ProfilesFile {
    if (existsSync(this.indexPath)) {
      try {
        const parsed = JSON.parse(readFileSync(this.indexPath, 'utf-8')) as ProfilesFile
        if (parsed.profiles?.length && parsed.profiles.some((p) => p.id === parsed.activeProfileId)) {
          return parsed
        }
      } catch {
  ...
  /**
   * First run on this profiles-aware version, or a fresh install: create the
   * default "Гостевой" profile. If a pre-profiles flat config.json exists
   * (an install from before this feature), adopt it as that profile's
   * settings instead of starting empty.
   */
  private migrateOrCreateDefault(userDataDir: string): ProfilesFile {
    ...
    const file: ProfilesFile = { activeProfileId: defaultProfile.id, profiles: [defaultProfile] }
    this.data = file
    this.save()
    return file
  }
  ...
  create(name: string): Profile {
    ...
    const profile: Profile = {
      ...
      avatarColor: AVATAR_COLORS[this.data.profiles.length % AVATAR_COLORS.length],
      createdAt: Date.now()
    }
  ...
  /** Returns true if the deleted profile was the active one (caller should relaunch). */
