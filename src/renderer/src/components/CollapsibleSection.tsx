...
interface CollapsibleSectionProps {
  ...
  defaultOpen?: boolean
  /**
   * Persists the open/closed state across app restarts under
   * `obscure:collapsibleSection:<persistKey>` (the same 'obscure:*'
   * localStorage convention as ThemeProvider/I18nProvider). When set, this
   * takes over from `defaultOpen` for the *initial* state, but a user who
   * has expanded a section keeps seeing it expanded on the next launch.
   */
  persistKey?: string
  className?: string
...
export function CollapsibleSection({
  ...
  const openProps = persistKey ? { open, onOpenChange: setOpen } : { defaultOpen }

  return (
    <Collapsible data-tour={tourId} {...openProps} className={cn('flex flex-col gap-3', className)}>
