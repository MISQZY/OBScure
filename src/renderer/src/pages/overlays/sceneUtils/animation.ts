...
export function maxExitDurationMs(nodes: Node[], edges: Edge[]): number {
  ...
  const visit = (n: Node): void => {
    ...
    if (n.type === 'box' || n.type === 'group') {
      for (const child of mods.filter((m) => m.type === 'text' || m.type === 'image' || m.type === 'video' || m.type === 'progress' || m.type === 'equalizer' || m.type === 'box' || m.type === 'group')) {
        visit(child)
      }
    }
  }
  const renderable = incoming(scene.id, edges, map).filter((n) => n.type === 'box' || n.type === 'group' || n.type === 'text' || n.type === 'image' || n.type === 'video' || n.type === 'progress' || n.type === 'equalizer')
